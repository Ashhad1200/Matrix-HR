// FIXTURE: a second tenant with the same shape of data as Acme, used by
// scripts/tenant-isolation-test.ts to prove one tenant cannot reach another's records.
// Generated from seed.ts (acme -> globex); safe to re-run. Login: admin@globex.com / Password123!
...
async function main() {
  console.log('Seeding Globex Software tenant...');

  const tenant = await prisma.tenant.upsert({
    where: { subdomain: 'globex' },
    update: {},
    create: {
  ...
  const adminUser = await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'admin@globex.com' } },
    update: {},
    create: {
      tenantId: tenant.id,
  ...
  const hrUser = await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'hr@globex.com' } },
    update: {},
    create: {
      tenantId: tenant.id,
      email: 'hr@globex.com',
  ...
  await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'ali.khan@globex.com' } },
  ...
  await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'sara.ahmed@globex.com' } },
  ...
  console.log('  Tenant: globex.matrixhr.com');
  console.log('  Admin: admin@globex.com / Password123!');
  console.log('  HR: hr@globex.com / Password123!');
  console.log('  Manager: ali.khan@globex.com / Password123!');
  console.log('  Employee: sara.ahmed@globex.com / Password123!');
}
